...
let books = {
  american_kingpin: {
    ...
    author: 'Nick Bilton',
    image:
      'https://images-na.ssl-images-amazon.com/images/I/51thUQ3znCL._SX329_BO1,204,203,200_.jpg',
  ...
  oliver_twist: {
    ...
    image:
      'https://images-na.ssl-images-amazon.com/images/I/51xKmBrsE%2BL._SX331_BO1,204,203,200_.jpg',
    checked_out: null,
    reviews: [
      {
        id: 4,
  ...
  the_art_of_war: {
    ...
    author: 'Sun Tzu',
    image:
      'https://images-na.ssl-images-amazon.com/images/I/51HmF37IInL._SX311_BO1,204,203,200_.jpg',
...
const bookFields = ['id', 'author', 'title', 'image', 'checked_out', 'reviews', 'description'];
...
export function _getUsers() {
  return new Promise((res, rej) => {
    setTimeout(() => res({ ...users }), generateResponseTime(0.2, 1000, 2000));
  });
}

export function _getBooks() {
  return new Promise((res, rej) => {
    setTimeout(() => res({ ...books }), generateResponseTime(0.2, 1000, 2000));
  });
}

function generateID(title) {
  return slugify(title.split(':')[0], { replacement: '_', lower: true });
}
...
function returnBook(user, bookId) {
  users = {
    ...
    [user.id]: {
      ...user,
      checked_out: user.checked_out.filter(id => id !== bookId),
    },
  };
}
...
function checkoutBook(user, bookId) {
  users = {
    ...
    [user.id]: {
      ...user,
      checked_out: user.checked_out.filter(id => id !== bookId).concat(bookId),
    },
  };
}
...
function validateBook(book) {
  ...
  const missingKeys = bookFields.filter(key => bookKeys.indexOf(key) < 0);
  const valid = missingKeys.length === 0;
  return { valid, missingKeys };
}

export function _createBook(book) {
  return new Promise((res, rej) => {
...
export function _updateBook(book) {
  return new Promise((res, rej) => {
    let { valid, missingKeys } = validateBook(book);
    if (!valid) {
      return rej({
